...
- **Start in a worktree**, and start it with `pnpm agent wt new <name> --type feat`. That creates the worktree and branch off `origin/develop`, sets the machine-account identity in it, reserves a **port slot**, writes that slot's ports into the worktree's own `.env` files, and installs dependencies. `git worktree add` by hand does none of that ; a worktree without a slot will fight a running session for port 3000. `pnpm agent wt ls` shows every worktree with its slot, `pnpm agent wt rm <name>` removes one and hands the slot back.
- **Use your own ports.** Every checkout owns a numbered slot, and slot N means web `3000 + N*10` and api `4000 + N*10` ; the primary checkout keeps slot 0, so `3000`/`4000` are not yours. The session is told its slot at startup and `pnpm dev` picks it up with no flags ; use those ports in every URL, `curl`, browser check and screenshot. `/ports` re-prints them, and `pnpm agent ports` shows what everyone else holds. Supabase is the exception : one shared local instance on `54321`, run from the main checkout, so a task with divergent migrations belongs there rather than in a worktree.
- **Commit and push as the agent, not the operator.** Every agent-authored commit and PR uses the dedicated `monark-agent` bot account, never the human operator's personal GitHub identity — that's the whole point of the machine-account attribution work (changelog `2026-08-16`): it lets agent PRs be reviewed and approved by a human instead of blocked as self-approval. [.github/CODEOWNERS](.github/CODEOWNERS) routes review away from the account, and [.github/workflows/agent-attribution.yml](.github/workflows/agent-attribution.yml) fails any PR opened by `monark-agent` that contains a commit authored by someone else — the exact failure mode of an agent silently falling back to the operator's stored credentials. Auth is the `MONARK_AGENT_TOKEN` env var (a GitHub PAT scoped to this repo, push + PR, no admin) — read it from the environment, never hardcode it, never log it, never let it land in a commit.

  None of this is a convention you have to remember any more ; it is enforced. `pnpm agent wt new` sets `user.name "Chrysa"` and `user.email "317500139+monark-agent@users.noreply.github.com"` in the checkout (local, never `--global`, so the operator's own config is untouched), that noreply address is what lets GitHub and `agent-attribution.yml` resolve a commit back to the `monark-agent` login, and a `git commit` with any other identity is refused before it happens.
  - **Ship with `pnpm agent pr`** (or `/ship`, which gates first). It pushes over HTTPS with `MONARK_AGENT_TOKEN` and `-c credential.helper=`, so Git Credential Manager cannot substitute the operator's cached login, and the token never lands in `.git/config` because `origin` is never rewritten. Then it opens the PR through the REST API with the same token, so the PR's `user.login` is `monark-agent` too, not just its commits. It refuses to push a branch carrying a commit authored by anyone else, which catches the mismatch before the push rather than in CI.
  - **`git push` is blocked outright**, by the guard described below. Don't work around it ; that block is the reason a fallback to the operator's credentials cannot happen by accident.
  - `gh` is usable, but only ever on the machine token : export `GH_TOKEN=$MONARK_AGENT_TOKEN` in the shell that launches the session, and any `gh` command without it is refused. That is stricter than it sounds for a reason : a freshly installed `gh` on this machine already found the operator's account in the OS keyring, left there by GitHub Desktop, so "don't run `gh auth login`" was never enough on its own. `GH_TOKEN` beats the keyring ; `gh auth` itself stays blocked so no login can be stored or switched from inside a session.
...
- **A policy guard sits in front of every command**, wired as a `PreToolUse` hook in [.claude/settings.json](.claude/settings.json) and defined in [.claude/agentkit/policy.json](.claude/agentkit/policy.json). Ordinary work runs without a prompt ; irreversible or identity-relevant things are refused with a reason (pushes, force-pushes, global git config, `reset --hard`, `clean -f`, database resets, `publish`, `rm -rf` outside the tree, naming the token in a command). A refusal is information, not an obstacle to route around : if the policy is wrong, say so and let the operator widen it in `.claude/policy.local.json`. The guard, the policy and `.claude/settings.json` are deliberately not agent-editable.
- **Unattended runs**: `pnpm agent night on` plus `claude --permission-mode dontAsk` turns every "ask the human" into an immediate refusal with a reason, so nothing can sit on a dialog overnight. `/overnight` drains `AGENT-QUEUE.md`, one worktree and one draft PR per task. If you are blocked in night mode, record it in the queue file and move on ; never idle waiting for an answer that is not coming.
...
- **Open a PR, don't push to `develop` directly.** Draft PRs early are welcome — opening one as soon as the branch exists gives everyone else (human or agent) visibility into what's in flight, which is the cheapest way to avoid two agents building the same thing twice. `pnpm agent pr` opens it as a draft.
...
- **Git hooks are shared across worktrees and live in the _main_ checkout's `.git/hooks`.** `simple-git-hooks` can't write them from a linked worktree (`.git` there is a file, not a directory), so after changing the `simple-git-hooks` block in [package.json](package.json) you must re-run `pnpm install` (or `pnpm prepare`) **in the main checkout** for the change to take effect. The `pre-push` hook deliberately unsets `GIT_DIR` / `GIT_WORK_TREE` / `GIT_INDEX_FILE` / `GIT_PREFIX` before running `pnpm lint` : git sets those when invoking a hook, and turbo's `git` child processes hang indefinitely if they inherit them (see changelog `2026-09-07`). Don't remove the `env -u` prefix.
...
- [ ] Pre-PR gate passes: `pnpm agent gate`
