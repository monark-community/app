...
- Reference : `listProjects` / `listIndustries` in [packages/projects/src/server/data.ts](packages/projects/src/server/data.ts). Consumers that genuinely need "all rows" (a filter dropdown) request an explicit high `limit` and read `.items` — they don't get a separate unbounded method.
