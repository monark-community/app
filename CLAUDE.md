...
- **Start in a worktree.** `claude --worktree <name>` (or `-w <name>`) creates an isolated worktree + branch and starts the session inside it — built into the CLI, no extra tooling needed. `spawn-new.bat` (in `claude-sessions/` at the monark root) does this automatically for a new persistent agent.
- **Branch naming**: `<type>/<slug>` — `feat/`, `fix/`, `chore/`, `docs/`, `tooling/`, matching the prefixes already in use ; `agent/<slug>` is what `spawn-new.bat --app` auto-creates before a task is scoped, rename it once you know what type it is. One task, one branch.
