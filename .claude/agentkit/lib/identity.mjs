...
export function ensureIdentity(cfg, cwd, { scope = "local" } = {}) {
  const want = cfg.identity ?? {};
  if (!want.email || !want.name)
    throw new Error("identity.name / identity.email are not set in agentkit.config.json");
  if (scope === "worktree") {
...
export function pushAsAgent(cfg, cwd, { branch, force = false } = {}) {
  ...
  if (!head || head === "HEAD")
    return { ok: false, reason: "detached HEAD; check out a branch first" };
  if (isProtected(cfg, head))
    return {
      ok: false,
      reason: `${head} is a protected branch; push a topic branch and open a PR`,
    };
