...
async function api(cfg, method, urlPath, body) {
  ...
  if (!res.ok) {
    ...
    const details = json?.errors ? ` (${json.errors.map((e) => e.message || e.code).join('; ')})` : ''
...
export async function findPr(cfg, repo, head) {
  ...
  const list = await api(cfg, 'GET', `/repos/${repo}/pulls?head=${owner}:${encodeURIComponent(head)}&state=open`)
