  ...
  switch (cmd) {
    case "ports": {
      ...
      if (sub === "prune") {
        ...
        log(
          dropped.length
            ? `freed ${dropped.length} slot(s): ${dropped.map((d) => d.slot).join(", ")}`
            : "nothing to prune",
        );
        return;
      }
    ...
    case "wt": {
      if (sub === "ls" || !sub) {
        ...
        for (const entry of wt.list(cfg)) {
          ...
          log(
            `  ${String(slot?.slot ?? "?").padStart(2)}  ${ports.padEnd(12)} ${path.basename(entry.dir).padEnd(28)} ${c.dim(entry.branch)}`,
          );
        }
        return;
      }
      if (sub === "new") {
        const name = argv[2];
        if (!name)
          die("usage: agentkit wt new <name> [--type feat] [--branch <branch>] [--base <branch>]");
        ...
        log(
          `  slot ${c.bold(String(res.slot))}: ${Object.entries(res.ports)
            .map(([k, v]) => `${k} ${v}`)
            .join(", ")}`,
        );
    ...
    case "identity": {
      ...
      if (sub === "fix") {
        const id = ensureIdentity(cfg, cwd, {
          scope: flag("scope", cfg.identity?.scope || "local"),
        });
      ...
      log(
        `  commits as   ${id.ok ? c.green(`${id.name} <${id.email}>`) : c.red(`${id.name} <${id.email || "unset"}>`)}`,
      );
    ...
    case "gate": {
      ...
      if (!checks.length)
        die("no checks configured; add a `checks` array to .claude/agentkit.config.json");
    ...
    case "night": {
      ...
      if (sub === "on") {
        ...
        log(
          c.green("night mode ON"),
          c.dim("(guard denies instead of prompting; nothing can hang)"),
        );
        log("");
    ...
    case "doctor": {
      ...
      const mine = reserve(cfg, here);
      log(
        `  slot         ${mine.slot} -> ${Object.entries(mine.ports)
          .map(([k, v]) => `${k}:${v}`)
          .join(" ")}`,
      );
      ...
      if (stale.length)
        problems.push(`${stale.length} stale slot(s); run \`agentkit ports prune\``);

      const id = identityMatches(cfg, here.dir);
      if (!id.ok)
        problems.push(
          `commit identity is ${id.email || "unset"}, expected ${cfg.identity.email}; run \`agentkit identity fix\``,
        );
      ...
      if (!tok.ok) problems.push(tok.reason);
        ...
        try {
          ...
          if (cfg.identity.name && me.login && !cfg.identity.email.includes(me.login)) {
            problems.push(
              `token belongs to ${me.login}, which does not match identity.email ${cfg.identity.email}`,
            );
          }
        } catch (err) {
      ...
      if (!fs.existsSync(hooks))
        problems.push(
          ".claude/settings.json is missing; the guard and session hooks are not wired up",
        );
