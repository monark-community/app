import { getFlagDef, parseFlagKey, type FlagScope } from "../contracts/index";
 ...
 */
export function mostSpecific(overrides: OverrideRow[], scope: FlagScope): OverrideRow | undefined {
  const byUser = scope.userId ? overrides.find((o) => o.userId === scope.userId) : undefined;
  if (byUser) return byUser;

  const byRole = scope.roleId ? overrides.find((o) => o.roleId === scope.roleId) : undefined;
...
export async function isEnabled(key: string, scope: FlagScope = {}): Promise<boolean> {
