...
import { _resetFlagRegistryForTesting, registerFlags } from "../../src/contracts/index";
...
// `beforeAll` seeded `Organization` + `User` rows that the per-test
// `afterEach` truncate doesn't touch. Clean them on suite exit so the
// rows don't outlive the testcontainer — defensive ; the container is
// destroyed anyway, but if someone bypasses globalSetup the
// `assert-test-db` setupFile aborts before this runs, so we never
// leak to a real DB.
afterAll(async () => {
  const db = getDb();
...
describe("feature-flags/resolve isEnabled — scoped precedence", () => {
  ...
  it("org-scoped override beats global when no user override matches", async () => {
    ...
    expect(await isEnabled("test.off-by-default", { organizationId: ORG_A })).toBe(true);
  });
  ...
  it("user-scoped override at one user does NOT leak to a different user", async () => {
    ...
    expect(await isEnabled("test.off-by-default", { userId: USER_A })).toBe(true);
    expect(await isEnabled("test.off-by-default", { userId: USER_B })).toBe(false);
  });
  ...
  it("org-scoped override does NOT leak across orgs", async () => {
    ...
    expect(await isEnabled("test.off-by-default", { organizationId: ORG_A })).toBe(true);
    expect(await isEnabled("test.off-by-default", { organizationId: ORG_B })).toBe(false);
  });
});
...
describe("feature-flags/resolve getFlags — multi-key batching", () => {
  ...
  it("returns false for an unknown flag passed alongside known ones", async () => {
    const result = await getFlags(["test.on-by-default", "test.does-not-exist"], {});
    expect(result).toEqual({
      "test.on-by-default": true,
