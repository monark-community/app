...
export async function listPendingInvitesForOrg(organizationId: string): Promise<
  Array<
    InviteRow & {
...
export async function findPendingInvitesForEmail(email: string): Promise<InviteRow[]> {
  const db = getDb();
  const now = new Date();
  return db.invite.findMany({
    where: {
