...
// Providers disagree on which key holds the human's name : most send
// `full_name` or `name`, GitHub falls back to `user_name` when the
// profile has no real name set, and OIDC providers use
// `preferred_username`. First non-empty wins. Kept deliberately broad
// so adding a provider needs no change here.
const DISPLAY_NAME_KEYS = ["full_name", "name", "user_name", "preferred_username"];
// `avatar_url` is the common spelling ; OIDC providers use `picture`.
