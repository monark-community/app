...
AUTH_OAUTH_PROVIDERS=google,azure,github
```
...
2. **Supabase** holds the credentials, through the `[auth.external.*]` stanzas in [`supabase/config.toml`](../../supabase/config.toml) locally (flip `enabled` to true ; the id + secret come from `env()` so nothing credential-shaped is committed) or Authentication → Providers in a hosted project's dashboard.
   ...
   Two things bite here, both verified the hard way:

   - **The `env()` refs resolve from the Supabase CLI's own environment**, i.e. the shell you run `supabase start` in, or a `.env` in that directory. `services/api/.env` is *not* read by the CLI. Put the provider credentials in a root `.env` (gitignored) with the exact names the stanza references, or export them before starting. When the CLI can't resolve one it does not fail loudly ; it forwards the literal string `env(SUPABASE_AUTH_EXTERNAL_…)` to GoTrue, and sign-in fails much later with an opaque provider error. `docker exec supabase_auth_app sh -c 'echo ${#GOTRUE_EXTERNAL_GITHUB_CLIENT_ID}'` reading 44 instead of ~20 is that placeholder.
   - **`redirect_uri` has to be set explicitly.** The upstream config template ships it as `""` and documents it as an optional override, but CLI 2.98.2 never substitutes a default: an empty value *and* an absent key both leave `GOTRUE_EXTERNAL_<PROVIDER>_REDIRECT_URI` empty in the auth container, and every authorize call fails with `validation_failed: Unsupported provider: missing redirect URI`. The stanzas here set it to `http://127.0.0.1:54321/auth/v1/callback` (this file is the local-dev config ; hosted projects derive it themselves). Verify with `curl -sD - "http://127.0.0.1:54321/auth/v1/authorize?provider=github" | head -1` — a `302` to the provider means it's wired, a `400` means it isn't.
...
| --------- | -------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Google    | `google` | Google Cloud console → OAuth consent screen + Credentials (Web application) | App verification is only required for sensitive scopes ; plain email + profile needs none. A local sign-in that fails on a nonce mismatch wants `skip_nonce_check = true` in the local config only.                                                                                            |
| Microsoft | `azure`  | Entra ID → App registrations                                                | Decide the tenant up front: the default multi-tenant endpoint accepts work **and** personal accounts, single-tenant needs `url = https://login.microsoftonline.com/<tenant-id>/v2.0`. The client secret **expires** (24 months maximum) ; put its renewal on a calendar or sign-ins stop dead. |
...
Apple is deliberately not wired. It needs a paid developer account and a client secret that is a JWT requiring regeneration every six months, and it only returns the user's name on the _first_ authorization ; worth doing as its own piece of work rather than riding along here.
...
- **Linking a provider to an existing session.** Today a provider gets linked only by Supabase's own verified-email matching at sign-in time. An explicit "connect Google to this account" button from `/account/security` (Supabase's `linkIdentity`) is the natural next step, and it's what the `email-collision` refusal currently tells the user to go and do.
- **Sign in with Apple.** Deliberately out of this pass: a paid developer account, a client secret that is a JWT needing regeneration every six months, and a name that is only returned on the first authorization. Its own piece of work.
