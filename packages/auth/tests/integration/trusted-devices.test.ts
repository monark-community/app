...
const UA_FIREFOX = "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:121.0) Gecko/20100101 Firefox/121.0";
...
describe("revokeTrustedDevice", () => {
  ...
  it("is idempotent — calling twice doesn't bump revokedAt forward", async () => {
    ...
    await revokeTrustedDevice({ userId: USER_A, deviceId: created.device.id });
    const db = getDb();
    const firstStamp = (await db.trustedDevice.findUnique({ where: { id: created.device.id } }))
      ?.revokedAt;
    await revokeTrustedDevice({ userId: USER_A, deviceId: created.device.id });
    const secondStamp = (await db.trustedDevice.findUnique({ where: { id: created.device.id } }))
      ?.revokedAt;
