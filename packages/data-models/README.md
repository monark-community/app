...
- `/server` — Prisma data layer (`data.ts`: models + fields + records + model-integrations CRUD, key/slug derivation and collision helpers, title derivation), `data-models.{manage-schema,read-schema,record-read,record-write,record-bulk-write,record-delete}` permissions (`permissions.ts`), the `dataModelsRouter` tRPC sub-router (`router.ts`), and event-type registration (`event-types.ts`).
...
Prisma models live under `// ── MODULE: data-models ──` in `packages/db/prisma/schema.prisma`: `DataModel`, `DataField`, `DataRecord`, `DataModelIntegration`, `DataFieldIndex`, `DataRecordRoleAccess`, `DataRecordWatcher`, `DataModelWatcher`. The `DataFieldType` enum gained a `FORMULA` member (computed, read-only field). Migrations: `20260706000312_add_data_models` … `20260708020000_add_formula_field_type`.
...
| `models.update`              | `data-models.manage-schema`          | name / description / icon                                                                                                                                                                                                                                             |
...
| `records.list`               | `data-models.record-read`            | per-model `<key>-record-read` OR generic (all record rows) + row-level filter ; `search` (title/slug) + `fieldFilters` (per-field predicates against the JSONB `data`: text/date substring, number/boolean equality, single-select any-of, multi-select contains-any) |
