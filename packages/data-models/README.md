...
- **Multi-tenant-safe per-model registration.** Per-model permissions + event types live in the process-global RBAC / event-type registries, and the readers (`/admin/rbac` catalog, webhook picker) list them unfiltered. Under the current single-tenant deploy that's correct — one org, so the global set IS that org's set. A true multi-tenant deploy would leak one org's model keys into every org's picker ; the fix is to filter the `data-models.<key>-*` entries by the caller's org in those readers. Relatedly, a **hard-deleted** model's registry entries linger until the next boot re-hydrates only live models (a soft-deleted model's do too, which is intentional — restore should bring them back).
