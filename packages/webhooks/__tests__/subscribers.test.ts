...
vi.mock("../src/server/secrets", () => ({
  computeIdempotencyKey: vi
    .fn()
    .mockReturnValue("mock-idem-key-000000000000000000000000000000000000000000000000"),
}));
...
describe("event routing", () => {
  ...
  it("platform-tier endpoint (orgId=null) receives every matching event", async () => {
    ...
    expect(enqueueDeliveries).toHaveBeenCalledWith(
      expect.objectContaining({
        matches: expect.arrayContaining([expect.objectContaining({ endpointId: "ep-platform" })]),
  ...
  it("org-scoped endpoint receives event with matching organizationId", async () => {
    ...
    expect(enqueueDeliveries).toHaveBeenCalledWith(
      expect.objectContaining({
        matches: expect.arrayContaining([expect.objectContaining({ endpointId: "ep-org" })]),
      }),
    );
  });
