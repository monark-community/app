...
describe("webhooks/worker deliverOne — exhausted retries", () => {
  it("marks delivery failed{permanent:true} + auto-disables endpoint after the failure limit", async () => {
    ...
    const disabled = events.find(
      (e) => e.type === "webhook.endpoint-disabled-after-failures",
    )
    expect(disabled).toBeDefined()
    expect(
      (disabled!.payload as { consecutiveFailures: number }).consecutiveFailures,
    ).toBe(WEBHOOK_DELIVERY_FAILURE_LIMIT)
  })
