...
import { validateColor, validatePermissionList, validateRoleKey } from "./validators";
...
export async function assignRole(input: {
  ...
  if (role.builtIn) {
    ...
    if (role.organizationId === null) {
      throw new ValidationError("Custom roles must declare an organizationId.");
    }
...
export async function deleteRole(input: { id: string; actorId: string }): Promise<void> {
  const role = await findRoleById(input.id);
  if (!role) throw new NotFoundError("Role", input.id);
  if (role.builtIn) {
