...
describe("<RoleChip>", () => {
  ...
  it("falls back to muted styling when color is null", () => {
    const { container } = renderWithIntl(<RoleChip name="Moderator" color={null} />);
  ...
  it("falls back to muted styling when color is undefined", () => {
    const { container } = renderWithIntl(<RoleChip name="Editor" color={undefined} />);
    const chip = container.firstChild as HTMLElement;
    expect(chip.className).toContain("bg-muted");
  });

  it("uses the color tint when a hex color is supplied", () => {
    const { container } = renderWithIntl(<RoleChip name="Lead" color="#F0870C" />);
    const chip = container.firstChild as HTMLElement;
  ...
  it("merges any custom className without dropping the base classes", () => {
    const { container } = renderWithIntl(
      <RoleChip name="Viewer" color={null} className="ring-2 ring-primary" />,
    );
    const chip = container.firstChild as HTMLElement;
  ...
  it("renders an X-icon remove button when onRemove is provided", () => {
    ...
    expect(screen.getByRole("button", { name: "Revoke Reviewer" })).toBeInTheDocument();
  });
