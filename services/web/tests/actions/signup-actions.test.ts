...
vi.mock("@/lib/supabase/server", () => ({
  createSupabaseServerClient: async () => ({
    auth: {
      signInWithPassword: (i: { email: string; password: string }) => mockSignInWithPassword(i),
    },
  }),
}));
...
describe("signUpAction", () => {
  it("redirects to check-email when needsEmailVerification is true", async () => {
    await expect(signUpAction({ email: "u@x.io", password: "strong-password-1" })).rejects.toThrow(
  ...
  it("forwards the request's accept-language to the API", async () => {
    ...
    expect(mockSignUp).toHaveBeenCalledWith(expect.objectContaining({ localePreference: "fr" }));
  });
  ...
  it("includes the optional displayName in the API call when provided", async () => {
    ...
    expect(mockSignUp).toHaveBeenCalledWith(expect.objectContaining({ displayName: "Test User" }));
  });
  ...
  it("skips notifySignedIn when auto-sign-in fails (verification still pending)", async () => {
    ...
    await expect(signUpAction({ email: "u@x.io", password: "strong-password-1" })).rejects.toThrow(
      "__redirect__:/signup/check-email",
    );
