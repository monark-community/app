...
vi.mock("@/lib/trpc-server", () => ({
  createServerTrpcClient: (_token?: string) => ({
    auth: {
      totp: {
        isChallengeRequired: {
          query: (input: { trustedDeviceId: string | null }) => mockIsChallengeRequired(input),
        },
      },
...
import { signInAction, signOutAction } from "@/app/(anon)/signin/actions";
...
describe("signInAction", () => {
  it("redirects to /account on a successful no-TOTP sign-in", async () => {
    await expect(signInAction({ email: "u@x.io", password: "pw" })).rejects.toThrow(
  ...
  it("redirects to /signup/check-email on email_not_confirmed", async () => {
    ...
    await expect(signInAction({ email: "u@x.io", password: "pw" })).rejects.toThrow(
      "__redirect__:/signup/check-email?email=u%40x.io",
    );
  });
  ...
  it("URL-encodes the email before redirecting to check-email", async () => {
    ...
    await expect(signInAction({ email: "user+tag@x.io", password: "pw" })).rejects.toThrow(
      "__redirect__:/signup/check-email?email=user%2Btag%40x.io",
    );
  });
...
describe("signOutAction", () => {
  ...
  it("forwards the scope to both the emit + Supabase signOut", async () => {
    await expect(signOutAction("global")).rejects.toThrow("__redirect__:/signin");
