...
vi.mock("@/lib/trpc-server", () => ({
  createServerTrpcClient: (_t?: string) => ({
    auth: {
      ...
      checkPassword: {
        mutate: (i: { password: string; email: string }) => mockCheckPassword(i),
      },
...
describe("resetPasswordAction", () => {
  it("redirects to /signin?passwordReset=1 on the happy non-TOTP path", async () => {
    await expect(resetPasswordAction({ newPassword: "new-strong-password" })).rejects.toThrow(
      "__redirect__:/signin?passwordReset=1",
    );
  ...
  it("passes the user's email into the strength check (rule context)", async () => {
    ...
    await expect(resetPasswordAction({ newPassword: "alice-pass-x" })).rejects.toThrow(
      "__redirect__:/signin?passwordReset=1",
    );
