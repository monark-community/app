...
vi.mock("@/lib/supabase/admin", () => ({
  createSupabaseAdminClient: () => ({
    storage: {
      from: (_bucket: string) => ({
        upload: (path: string, body: unknown, opts: unknown) => mockAdminUpload(path, body, opts),
...
function makeFormData(input: { file?: unknown; orgId?: unknown }): FormData {
...
describe("adminUploadOrgLogoAction", () => {
  it("returns ok with the public URL on the happy path", async () => {
    ...
    expect(mockAdminUpdate).toHaveBeenCalledWith(expect.objectContaining({ id: "org-1" }));
  });

  it("uploads to org-logos/<orgId>/<ts>.webp", async () => {
    await adminUploadOrgLogoAction(makeFormData({ file: makeFile(), orgId: "org-7" }));
  ...
  it("returns upstream when orgId is missing", async () => {
    const result = await adminUploadOrgLogoAction(makeFormData({ file: makeFile() }));
  ...
  it("returns invalidType when no file is supplied", async () => {
    const result = await adminUploadOrgLogoAction(makeFormData({ orgId: "org-1" }));
  ...
  it("returns tooLarge when the file exceeds 2 MB", async () => {
    ...
    const result = await adminUploadOrgLogoAction(makeFormData({ file: bigFile, orgId: "org-1" }));
  ...
  it("appends the cache-buster timestamp + passes the same URL to adminUpdate", async () => {
    await adminUploadOrgLogoAction(makeFormData({ file: makeFile(), orgId: "org-1" }));
