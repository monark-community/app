...
test.describe("sign-in happy path", () => {
  ...
  test("invalid credentials surface the error toast", async ({ page }) => {
    ...
    await page.getByLabel(/^password$|^mot de passe$/i).fill("definitely-wrong-pass");
    await page.getByRole("button", { name: /sign in|se connecter|connexion/i }).click();
    // The invalid-credentials path keeps the user on /signin and
    // surfaces an inline error.
    await expect(page).toHaveURL(/\/signin/);
    await expect(page.getByText(/email or password is incorrect|incorrect/i)).toBeVisible({
      timeout: 10_000,
    });
  });
});
