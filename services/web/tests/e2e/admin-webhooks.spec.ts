...
test.describe("admin webhooks — end-to-end delivery", () => {
  ...
  test(
    "create endpoint → trigger event → receiver captures signed delivery",
    async ({ page }) => {
      ...
      // the `rbac.` prefix.
      await page
        .getByRole("checkbox", { name: /toggle every event in rbac/i })
        .check()
      ...
      await expect(
        page.getByText(/endpoint created|point de terminaison créé/i),
      ...
      await page
        .getByLabel(/^description$/i)
        .fill("Created by the webhooks e2e ; safe to delete.")
      ...
      expect(
        capture.headers["webhook-delivery-idempotency-key"],
      ).toBeTruthy()
      ...
      await page
        ...
        .click()
      await expect(
        page.getByText(/endpoint deleted|point de terminaison supprimé/i),
      ).toBeVisible({ timeout: 10_000 })
    },
  )
})
