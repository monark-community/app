...
test.describe("password change", () => {
  ...
  test("change password, sign out, sign in with new, rotate back", async ({
    page,
  }) => {
    // 1. Sign in.
    ...
    await page
      .getByRole("button", { name: /change password|modifier le mot de passe/i })
      .click()

    // 3. Fill the modal.
    await page
      .getByLabel(/current password|mot de passe actuel/i)
      .fill(ORIGINAL_PASSWORD!)
    await page
      .getByLabel(/^new password$|^nouveau mot de passe$/i)
      .fill(TEMP_PASSWORD)
    await page
      .getByLabel(/confirm.*password|confirmer.*mot de passe/i)
      .fill(TEMP_PASSWORD)
    await page
    ...
    await page
      .getByRole("button", { name: /open user menu|ouvrir le menu/i })
      .click()
