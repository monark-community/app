...
export async function startReceiver(options?: {
  ...
  return {
    url,
    port,
    waitForCapture: (predicate, timeoutMs = 15_000) => waitForCapture(api, predicate, timeoutMs),
...
async function waitForCapture(
  ...
  throw new Error(`webhook-receiver: no capture matched the predicate within ${timeoutMs}ms`);
}

async function stopReceiver(child: ChildProcess, api: APIRequestContext): Promise<void> {
