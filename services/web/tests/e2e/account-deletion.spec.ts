...
test.describe("account deletion grace flow", () => {
  ...
  test(
    "request deletion → sign back in → see grace lockdown → cancel → tabs restored",
    async ({ page }) => {
      // 1. Sign in.
      ...
      await page
        .getByRole("button", { name: /request.*deletion|demander.*suppression/i })
        .click()

      // 3. Type the email to enable the confirm button + submit.
      await page
        .getByLabel(/type.*email|saisissez.*adresse/i)
        .fill(TEST_EMAIL)
      await page
        .getByRole("button", {
      ...
      await expect(
        page.getByText(/deletion scheduled|suppression programmée/i),
      ).toBeVisible()
      ...
      await expect(
        page.getByText(/cancelled|annulée/i),
      ).toBeVisible({ timeout: 10_000 })

      // 8. Sidebar should now show all 4 tabs again.
      await expect(
        page.getByRole("link", { name: /security|sécurité/i }),
      ).toBeVisible()
      await expect(
