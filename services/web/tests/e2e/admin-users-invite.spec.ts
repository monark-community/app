...
test.describe("admin users — invite flow", () => {
  ...
  test(
    "invite a user then revoke the pending invite",
    async ({ page }) => {
      ...
      await page
        .getByRole("button", { name: /invite user|inviter un utilisateur/i })
        .click()
      ...
      await page
        .getByLabel(/^full name$|^nom complet$/i)
        .fill(INVITE_NAME)
      ...
      // 5. Submit.
      await page
        .getByRole("button", { name: /^send invite$|^envoyer$/i })
        .click()
      await expect(
      ...
      await page
        .getByRole("button", {
          name: new RegExp(
            `(revoke invite for|révoquer l'invitation pour) ${INVITE_EMAIL}`,
            "i",
          ),
        })
        .click()
      await expect(
        page.getByText(/invite revoked|invitation révoquée/i),
      ).toBeVisible({ timeout: 10_000 })
