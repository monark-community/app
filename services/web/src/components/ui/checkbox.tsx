 ...
 * `<Check>` or `<Minus>` icon (`text-primary-foreground`, contrast-
 * tracked against the operator's brand color) when the box is checked
 * / indeterminate. The `indeterminate` prop is mirrored to
    ...
    >
      <input
        ...
        className={cn(
          ...
          filled ? "border-primary bg-primary" : "border-input hover:border-primary/60",
          className,
        )}
        {...props}
      />
      ...
      {indeterminate || checked ? (
        <span className="pointer-events-none absolute inset-0 flex items-center justify-center">
          {indeterminate ? (
            <Minus className="h-3.5 w-3.5 text-primary-foreground" strokeWidth={3} aria-hidden />
          ) : (
            <Check className="h-3.5 w-3.5 text-primary-foreground" strokeWidth={3} aria-hidden />
          )}
        </span>
      ) : null}
    </span>
