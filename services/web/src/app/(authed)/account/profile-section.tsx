...
export function ProfileSection() {
  ...
  const dirty = useMemo(() => {
    ...
    return false;
  }, [me.data, readOnly, displayName, bio, localeDraft, currentLocale, mounted, themeDraft, theme]);
  ...
  function onSave() {
    ...
    if (trimmedName !== namePrev) {
      if (trimmedName.length < 1 || trimmedName.length > 80) {
        toast.error(t("displayNameError"));
        return;
      }
      profilePatch.displayName = trimmedName;
    }
    if (Array.from(trimmedBio).length > BIO_MAX) return;
    if (trimmedBio !== bioPrev.trim()) {
      profilePatch.bio = trimmedBio.length === 0 ? null : trimmedBio;
    }

    const localeChanged =
      localeDraft !== currentLocale && (localeDraft === "en" || localeDraft === "fr");
    const themeChanged = mounted && themeDraft !== (theme ?? "system");
        ...
        <div className="flex items-start gap-3 rounded-lg border border-amber-400/40 bg-amber-400/5 p-3">
          ...
          <div className="flex-1 space-y-2">
            <div className="space-y-0.5">
              <p className="text-sm font-medium text-amber-500">{tBanner("title")}</p>
              <p className="text-xs text-muted-foreground">{tBanner("subtitle")}</p>
            </div>
            <Button
              type="button"
              variant="outline"
