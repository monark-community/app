...
export function SubscriptionPicker({
  ...
  function toggleEventInGroup(
    ...
    if (on) {
      ...
      onChange([
        ...subscriptions,
        { localId: nextLocalId(), eventType: type, isPrefix: false },
      ])
    } else {
      onChange(
        subscriptions.filter(
          (s) => !(s.eventType === type && s.isPrefix === false),
        ),
      )
    }
  }
  ...
  function toggleAllInGroup(
    ...
    if (prefix !== null) {
      ...
      onChange([
        ...cleared,
        { localId: nextLocalId(), eventType: prefix, isPrefix: true },
      ])
      return
    }
          ...
          filteredGroups.map((group) => {
            ...
            return (
              <ModuleSection
                ...
                onToggleExact={(type, next) =>
                  toggleEventInGroup(group, type, next)
                }
              />
            )
          })
              ...
              <li key={ev.type}>
                <label className="flex cursor-pointer items-start gap-2 rounded-md border border-border px-3 py-2 text-sm">
                  <Checkbox
                    checked={checked}
                    onChange={(event) =>
                      onToggleExact(ev.type, event.target.checked)
                    }
                    aria-label={ev.type}
                  />
