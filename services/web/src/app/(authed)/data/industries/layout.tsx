...
// parent Data layout. Bounces users without `industries.read` to `/`.
export default async function IndustriesLayout({ children }: { children: ReactNode }) {
  ...
  if (!(perms as string[]).includes("industries.read")) redirect("/");
  return <>{children}</>;
}
