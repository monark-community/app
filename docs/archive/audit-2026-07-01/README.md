...
> **Archived snapshot.** A point-in-time audit from 2026-07-01, moved to `docs/archive/` on 2026-08-04. It reflects the working tree as it was then ; many of the top findings have since been fixed (see the CHANGELOG). Links **between** the reports below still work ; outbound links (to `../features-planning/`, `../technical-documentation/`, `../../packages/`, …) were written relative to the original `docs/review/` location and point at the docs tree as it stood then — some targets (e.g. the features-planning specs) have since moved or been reorganized.
