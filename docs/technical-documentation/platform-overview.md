...
- **Three-layer authorization**: model layer (per-model `data-models.<key>-record-{read,write,delete}` OR the generic `data-models.record-*` ; admins short-circuit) then row layer (`DataRecordRoleAccess` ; a record with no rows is visible to all model-accessors ; restricted records 404 rather than 403 for unlisted callers). Bypassing the row layer is its own capability, `data-models.view-all-records`, deliberately **not** implied by `manage-schema` : designing a schema and reading every row in it are different powers, so a data steward can hold the first without the second. A third layer, **record scopes** (`DataModelRoleScope`), lets a role be granted "the records matching this MonarkQL query" per model ; roles stay additive, so one unscoped granting role leaves the caller unrestricted, and a scope that fails to parse denies rather than ceasing to filter. See [record-scopes.md](record-scopes.md).
